...
---
# 📊 Redis vs MySQL 성능 보고서

## 테스트 결과

### ▶️ Redis 성능
| VUser 수 | TPS | 가장 높은 TPS | 평균 턴스 시간 (ms) | 에러수 |
|:--|:--|:--|:--|:--|
| 99 | 1,649.7 | 2,149.5 | 56.89 | 0 |
| 1000 | 456.5 | 824.5 | 1,029.84 | 0 |
| 3000 | 45.7 | 200.5 | 4,112.23 | 560 |

### ▶️ MySQL 성능
| VUser 수 | TPS | 가장 높은 TPS | 평균 턴스 시간 (ms) | 에러수 |
|:--|:--|:--|:--|:--|
| 99 | 1,943.9 | 2,784.0 | 48.86 | 0 |
| 1000 | 822.3 | 1,428.5 | 616.68 | 0 |
| 3000 | 18.0 | 68.5 | 7,367.84 | 436 |

---

✅ 99명 (소규모 부하)
MySQL > Redis
➔ TPS, 평균 응답속도 모두 MySQL이 더 빠름. ➔ 거의 디스크 부하 없이 처리 가능.

✅ 1000명 (중간 부하)
MySQL ≈ Redis ➔ MySQL이 여전히 TPS는 높지만, 응답속도는 Redis가 약간 더 유리해지는 추세. ➔ 둘 다 에러는 없음.

✅ 3000명 (대규모 부하)
Redis 압승 ➔ MySQL은 커넥션 한계, 락 경합, 디스크 IO 한계로 TPS 폭락 (18 TPS) 발생. ➔ Redis는 여전히 45.7 TPS 유지. ➔ 에러는 둘 다 증가했지만, Redis가 에러율, TPS 모두 압도적으로 더 안정적.

### 3000VUser 에러율
|항목 | MySQL | Redis |
|:--|:--|:--|
|에러 절대 수 | 낮음 (436건) | 높음 (560건)|
|에러율(%) | 매우 높음 (66%) | 낮음 (5.6%)|
|총 처리량 (executed tests) | 655건 | 9,929건|

> 읽기 부하가 1000명을 넘어가면, 반드시 Redis와 같은 메모리 캐시 계층이 필요하다."

MySQL은 TPS 대비 부하에 급격히 취약해지고,
Redis는 메모리 기반이라 트래픽 급증에도 훨씬 안정적이다.

![Image](https://github.com/user-attachments/assets/56387dd3-a42c-493e-8af0-4292c4bf0d24)
